require_relative "../lib/calculator_772.rb"
require "rspec"

RSpec.describe Calculator772 do
  let(:subject) { Calculator772.new }
  ...
  describe "#calculate" do
    ...
    it "handles simple parentheses" do
      expect(subject.calculate("(2+3)*4")).to eq(20)
    end

    it "handles single parentheses expression" do
      expect(subject.calculate("(2+3)")).to eq(5)
    end

    it "handles simple parenthesis with multiple + and - within it before multiplication" do
      expect(subject.calculate("(2+3-1)*4")).to eq(16)
    end

    it "handles simple parenthesis with multiple + and - within it after multiplication" do
      expect(subject.calculate("2*(3+1)")).to eq(8)
    end

    it "handles simple parentheses with multiplication" do
      expect(subject.calculate("(1+2)*3")).to eq(9)
    end
  end
end
